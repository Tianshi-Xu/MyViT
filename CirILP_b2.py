...
def set_rotate_mat():
    ...
    for block_size in search_space:
        if block_size==1:
            continue
        ...
        rotate_mats[block_size] = rotate_mat
        rev_rotate_mats[block_size] = rev_rotate_mat

def trans_to_cir(weight: nn.Parameter,block_size):
    if block_size == 1:
        return weight
    # print("block_size:",block_size)
    rotate_mat = rotate_mats[block_size].cuda()
    rev_rotate_mat = rev_rotate_mats[block_size].cuda()
    out_features = weight.shape[0]
    in_features = weight.shape[1]
    q = out_features // block_size
    p = in_features // block_size
    if weight.dim()==4:
        kernel_size = weight.shape[2]
        tmp = weight.reshape(q, block_size, p, block_size, kernel_size,kernel_size)
        # tmp (q,p,b,b,1,1)
        tmp = tmp.permute(0, 2, 1, 3,4,5)
        # print(tmp[0,0,:,:])
        weights_rot = tmp[:,:, rotate_mat[:, 0], rotate_mat[:, 1],:,:] 
        weights_rot = weights_rot.view(q,p, block_size, block_size, kernel_size,kernel_size)
        # print("-----------")
        ...
        weights_cir = weights_cir.view(q,p, block_size, block_size, kernel_size,kernel_size)
        # print(weights_cir[0,0,:,:])
        weights_cir=weights_cir.permute(0,2,1,3,4,5).reshape(out_features,in_features, kernel_size,kernel_size)
    elif weight.dim()==2:
        tmp = weight.reshape(q, block_size, p, block_size)
        # tmp (q,p,b,b)
...
def ILP(args,test_loader,model,loss_fn):
    ...
    for i in range(num_variable):
        prob += (variable[f"b1_{i}"]+ variable[f"b2_{i}"]+ variable[f"b4_{i}"]+ variable[f"b8_{i}"]) == 1
    ...
    prob += sum(variable[f"b1_{i}"]*sensitivity_b1[i] + variable[f"b2_{i}"]*sensitivity_b2[i] + variable[f"b4_{i}"]*sensitivity_b4[i] +variable[f"b8_{i}"]*sensitivity_b8[i] for i in range(num_variable))
    ...
    for i in range(num_variable):
        ...
        block_size = value(variable[f"b1_{i}"])+value(variable[f"b2_{i}"])*2+value(variable[f"b4_{i}"])*4+value(variable[f"b8_{i}"])*8
