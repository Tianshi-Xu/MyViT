...
from .utils import group_product, group_add, normalization, get_params_grad, hessian_vector_product, orthnormal, group_product_without_sum
...
class hessian():
    ...
    def __init__(self, model, criterion, data=None, dataloader=None, cuda=True):
        ...
        params, gradsH, sizes = get_params_grad(self.model)
        self.params = params
    ...
    def dataloader_hv_product(self, v):
        ...
        eigenvalue = group_product_without_sum(THv, v)
        ...
        while computed_dim < top_n:
            ...
            for i in range(maxIter):
                ...
                if self.full_dataset:
                    ...
                    tmp_eigenvalue = group_product_without_sum(Hv, v)
                ...
                if eigenvalue == None:
                    ...
                    convergence = True
                    for v1,v2 in zip(eigenvalue,tmp_eigenvalue):
                        if abs(v1 - v2) / (abs(v1) + 1e-6) > tol:
                            convergence = False
                    if convergence:
                        break
                    else:
    ...
    def trace(self, maxIter=100, tol=1e-3):
        ...
        device = self.device
        trace_vhv = []
        trace = [0. for i in range(len(self.params))]
        ...
        for i in range(maxIter):
            ...
            if self.full_dataset:
                ...
                Hv = hessian_vector_product(self.gradsH, self.params, v)
            trace_vhv.append(group_product_without_sum(Hv, v))

            convergence = True
            for i in range(len(v)):
                tmp_traces = [t[i] for t in trace_vhv]
                tmp_trace = np.mean(tmp_traces)
                if abs(tmp_trace - trace[i]) / (abs(trace[i]) + 1e-6) > tol:
                    convergence = False
                    trace[i] = tmp_trace
            if convergence:
                return trace
